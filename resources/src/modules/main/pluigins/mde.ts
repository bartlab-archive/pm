        ...
        if (options.toolbar === undefined) {
            ...
            for (let key in self.toolbarBuiltInButtons) {
                if (self.toolbarBuiltInButtons.hasOwnProperty(key)) {
                    ...
                    if (
                        self.toolbarBuiltInButtons[key].default === true ||
                        (options.showIcons &&
    ...
    render(el?) {
        ...
        for (let key in options.shortcuts) {
            ...
            if (
                ...
                options.shortcuts[key] !== null &&
                self.bindings.hasOwnProperty(key) &&
                self.bindings[key] !== null
            ) {
                // ((key) => {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (items[i] === '|') {
                ...
                for (let x = i + 1; x < items.length; x++) {
                    if (
                        items[x] !== '|' &&
                        (!this.options.hideIcons || this.options.hideIcons.indexOf(items[x].name) === -1)
                    ) {
